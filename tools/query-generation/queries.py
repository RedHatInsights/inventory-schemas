class SelectQueries:    
    def string_query(self, field_name, operation, value):
        if operation == "max-length":
            query = f"SELECT reporter, CASE WHEN system_profile_facts->>'{field_name}' is null THEN 'null' WHEN LENGTH(system_profile_facts->>'{field_name}') < {value} THEN 'pass' ELSE 'fail' END case_result, count(id) FROM hosts GROUP BY reporter, case_result;"
        if operation == "pattern":
            query = f"SELECT reporter, CASE WHEN system_profile_facts->>'{field_name}' is null THEN 'n/a' WHEN (system_profile_facts->>'{field_name}') ~ '{value}' THEN 'pass' ELSE 'fail' END case_result, count(id) FROM hosts GROUP BY reporter, case_result;"
        return query
